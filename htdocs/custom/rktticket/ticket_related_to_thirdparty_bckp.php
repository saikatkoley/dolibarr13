...
$diroutputmassaction = $conf->rktticket->dir_output.'/temp/massgeneration/'.$user->id;
...
$url_page_current = dol_buildpath('/custom/rktticket/list.php', 1);
...
$permissiontoread = $user->rights->rktticket->read;
$permissiontoadd = $user->rights->rktticket->create;
$permissiontodelete = $user->rights->rktticket->delete;
...
if (empty($reshook))
{
	...
	$objectclass = 'Ticket';
	$objectlabel = 'Ticket';
	$uploaddir = $conf->rktticket->dir_output;
...
foreach ($search as $key => $val)
{
	if ($key == 'status' && !empty($search['status']))
	{
		...
		if ($search['status'] == 'openall' || in_array('openall', $search['status'])) {
			$newarrayofstatus[] = RktTicket::STATUS_NOT_READ;
			$newarrayofstatus[] = RktTicket::STATUS_READ;
			$newarrayofstatus[] = RktTicket::STATUS_ASSIGNED;
			$newarrayofstatus[] = RktTicket::STATUS_IN_PROGRESS;
			$newarrayofstatus[] = RktTicket::STATUS_NEED_MORE_INFO;
			$newarrayofstatus[] = RktTicket::STATUS_WAITING;
		}
		if ($search['status'] == 'closeall' || in_array('closeall', $search['status'])) {
			$newarrayofstatus[] = RktTicket::STATUS_CLOSED;
			$newarrayofstatus[] = RktTicket::STATUS_CANCELED;
		}
...
if ($search_date_start)			$sql .= " AND t.creation_date >= '".$db->idate($search_date_start)."'";
if ($search_date_end)			$sql .= " AND t.creation_date <= '".$db->idate($search_date_end)."'";


if (!$user->socid && ($mode == "mine" || (!$user->admin && $conf->global->TICKET_LIMIT_VIEW_ASSIGNED_ONLY))) {
	$sql .= " AND (t.assigned_to = ".$user->id;
	if (empty($conf->global->TICKET_LIMIT_VIEW_ASSIGNED_ONLY)) $sql .= " OR t.created_by = ".$user->id;
	$sql .= ")";
}
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all)
{
	...
	$id = $obj->rowid;
	header("Location: ".DOL_URL_ROOT.'/custom/rktticket/ticket.php?id='.$id);
	exit;
}
...
if ($socid && !$projectid && !$project_ref && $user->rights->societe->lire) {
	...
	if ($res > 0) {
		...
		print dol_get_fiche_head($head, 'rktticket', $langs->trans("ThirdParty"), -1, 'company');
...
$url = DOL_URL_ROOT.'/custom/rktticket/ticket.php?action=create'.($socid ? '&socid='.$socid : '').($projectid ? '&origin=projet_project&originid='.$projectid : '');
...
foreach ($object->fields as $key => $val)
{
	...
	if (!empty($arrayfields['t.'.$key]['checked'])) {
		if ($key == 'fk_type') {
			...
			print '</td>';
		} elseif ($key == 'assigned_to' || $key == 'created_by') {
			print '<td class="liste_titre'.($cssforfield ? ' '.$cssforfield : '').'">';
			...
			{
				if ($key2 == RktTicket::STATUS_CLOSED) $arrayofstatus['closeall'] = '-- '.$langs->trans('ClosedAll').' --';
...
{
	$cssforfield = (empty($val['css']) ? '' : $val['css']);
	if ($key == 'status' || $key == 'fk_severity') $cssforfield .= ($cssforfield ? ' ' : '').'center';
	elseif (in_array($val['type'], array('date', 'datetime', 'timestamp'))) $cssforfield .= ($cssforfield ? ' ' : '').'center';
	elseif (in_array($val['type'], array('timestamp'))) $cssforfield .= ($cssforfield ? ' ' : '').'nowrap';
	elseif (in_array($val['type'], array('double(24,8)', 'double(6,3)', 'integer', 'real', 'price')) && $val['label'] != 'TechnicalID') $cssforfield .= ($cssforfield ? ' ' : '').'right';
	if (!empty($arrayfields['t.'.$key]['checked']))
...
while ($i < min($num, $limit))
{
	...
	foreach ($object->fields as $key => $val)
	{
		...
		if (!empty($arrayfields['t.'.$key]['checked']))
		{
			...
			elseif ($key == 'fk_type') {
				$s = $dictlibelles[$object->fk_type];
				print '<span title="'.$s.'">';
				print $s;
				print '</span>';
			}
			elseif ($key == 'fk_category') {
				$s = $dictlibelles[$object->fk_category];
				print '<span title="'.$s.'">';
				print $s;
				print '</span>';
			}
			elseif ($key == 'fk_severity') {
				$s = $dictlibelles[$object->fk_severity];
				print '<span title="'.$s.'">';
				print $s;
				print '</span>';
			}
